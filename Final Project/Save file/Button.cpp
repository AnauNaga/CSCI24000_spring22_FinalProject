...
Button::Button(int x,int y, std::string text, unsigned int* parentBuffer, int buffer_width){
	...
	buffer_buttonUp = malloc(range.width * range.height * 4);
	buffer_buttonDown = malloc(range.width * range.height * 4);
	is_down = false;
	changed = false;
...
Button::~Button(){
	free(buffer_buttonUp);
	free(buffer_buttonDown);
}

void Button::pressButton(unsigned int* buffer){
	unsigned int* pixel = buffer;
	unsigned int* localpixel = (unsigned int*)buffer_buttonDown;
